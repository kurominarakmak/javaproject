...
public class Vortex extends Projectile {
    ...
    private static int totalFrames = 60; // Adjust based on the number of PNG frames
    private int currentFrame = 0;
    private double animationTimer = 0;
    private double frameDuration = 0.2; // Adjust for smooth animation
    ...
    static {
        ...
        for (int i = 0; i < totalFrames; i++) {
            String path = String.format("projectiles/vortex/Effect_TheVortex_1_%03d.png", i);
            frames[i] = new Image(ClassLoader.getSystemResourceAsStream(path));
        }
    }

    public Vortex(double x, double y, double targetX, double targetY, long now) {
